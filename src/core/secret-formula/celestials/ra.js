export const ra = {
  ...
  unlocks: {
    ...
    celestialDimensionPower: {
      ...
      effect: () => player.disablePostReality ? 1 : Math.pow(Math.clamp(Ra.pets.teresa.level * Math.log10(player.endgames + 1) / 2000, 1, 2) * Math.pow(Math.max(Ra.pets.teresa.level * Math.log10(player.endgames + 1) / 4000, 1), 0.2), 5),
      pet: "teresa",
      level: 125,
